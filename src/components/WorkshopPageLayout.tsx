...
const otherWorkshops = [
  { title: "Full Pottery Experience", link: "/workshop/pottery-experience" },
  { title: "Handbuilding Workshop", link: "/workshop/handbuilding" },
  { title: "Embroidery Workshop", link: "/workshop/embroidery" },
];
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 md:py-12">
        <div className="space-y-6">
          ...
          <div>
            {workshop.popular && (
              <span className="inline-block bg-cta text-primary-foreground text-xs font-semibold px-3 py-1 rounded-full mb-3">
                Most Popular
              </span>
            )}
            <h1 className="text-2xl md:text-3xl font-medium">{workshop.title}</h1>
            <p className="text-foreground/60 mt-2">{workshop.tagline}</p>
          </div>
          ...
          {workshop.images.length > 1 && (
            <div className="grid grid-cols-2 gap-3">
              {workshop.images.slice(1).map((img, i) => (
                <div key={i} className="aspect-square rounded-xl overflow-hidden">
                  <img src={img} alt={`${workshop.title} ${i + 2}`} className="w-full h-full object-cover" loading="lazy" />
                </div>
              ))}
            </div>
          )}
        ...
        <div className="mt-16 pt-8 border-t border-border/30">
          <h3 className="text-lg font-medium mb-4">Explore Other Workshops</h3>
            ...
            {otherWorkshops
              ...
              .map((w) => (
                <Link
                  key={w.link}
                  to={w.link}
                  className="p-4 rounded-xl bg-card/50 hover:bg-card transition-colors text-sm font-medium"
                >
                  {w.title} →
                </Link>
              ))}
