...
import { getBlogPost, blogPosts, BlogPost as BlogPostType } from "@/data/blogPosts";
...
const categoryLabels: Record<BlogPostType["category"], Record<string, string>> = {
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="py-12 px-5 bg-muted/30">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-muted-foreground mb-6">
            {language === "ar"
              ? "احجز ورشة فخار في تطوان واصنع ذكريات تدوم"
              : language === "es"
              ? "Reserva un taller de cerámica en Tetuán y crea recuerdos duraderos"
              : language === "fr"
              ? "Réservez un atelier de poterie à Tétouan et créez des souvenirs durables"
              : "Book a pottery workshop in Tetouan and create lasting memories"}
          </p>
          <Button asChild size="lg" className="bg-cta hover:bg-cta-hover">
            <Link to="/#booking">
              {language === "ar" ? "احجز مكانك" : language === "es" ? "Reserva tu lugar" : language === "fr" ? "Réservez votre place" : "Book Your Spot"}
            </Link>
          </Button>
        </div>
      </section>
                ...
                <Link key={related.slug} to={`/blog/${related.slug}`}>
                  <Card className="group overflow-hidden hover:shadow-lg transition-all h-full">
                    ...
                    <CardContent className="p-5">
                      <h4 className="font-semibold group-hover:text-cta transition-colors line-clamp-2">
      ...
      <div className="pb-16 px-5">
        <div className="max-w-3xl mx-auto">
          <Link
            to="/blog"
            className="inline-flex items-center gap-2 text-muted-foreground hover:text-cta transition-colors"
          >
            <ArrowLeft size={16} />
            {language === "ar"
              ? "العودة إلى المدونة"
