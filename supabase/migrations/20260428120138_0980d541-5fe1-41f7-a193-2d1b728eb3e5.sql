...
begin
  ...
  if service_key is null or length(service_key) = 0 then
    raise warning 'trigger_booking_app_emails skipped: missing service credential';
    return new;
  end if;

  perform net.http_post(
    url := functions_url,
    headers := jsonb_build_object('Content-Type', 'application/json', 'Authorization', 'Bearer ' || service_key, 'apikey', anon_key),
