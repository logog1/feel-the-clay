...
begin
  ...
  perform net.http_post(
    ...
    headers := jsonb_build_object('Content-Type', 'application/json', 'Authorization', 'Bearer ' || anon_key, 'apikey', anon_key),
    body := jsonb_build_object('type', 'booking', 'data', jsonb_build_object('skipPersist', true, 'bookingId', new.id, 'idempotencyKey', 'booking-notify-' || new.id::text, 'name', new.name, 'email', coalesce(new.email, ''), 'phone', coalesce(new.phone, ''), 'workshop', new.workshop, 'date', coalesce(new.booking_date, ''), 'participants', coalesce(new.participants, 1), 'city', coalesce(new.city, ''), 'sessionInfo', coalesce(new.session_info, ''), 'notes', coalesce(new.notes, '')))
  );
